...
public class InventoryButtonsManager {
  ...
  private Consumer<InventoryManagementButton> addToScreen;
  ...
  /**
   * Called by the platform module's screen-init event handler. The {@code addToScreen}
   * callback should add the button to the screen's widget list (e.g., via
   * {@code Screens.getButtons(screen).add(button)} on Fabric).
   */
  public void onScreenAfterInit(Screen screen, Consumer<InventoryManagementButton> addToScreen) {
  ...
  private void addButton(HandledScreen<?> screen, InventoryManagementButton button, boolean isPlayerInventory) {
    this.addToScreen.accept(button);
